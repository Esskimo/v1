var Weglot = function () {
    ...
    function textNodesUnder(el) {
        var n, a = [], b = [], walk = document.createTreeWalker(el, 4, null, false);
        var c = [];
        var d = [];
            ...
            for (var i = 0; i < inputs.length; i++) {
                ...
                if (!hasAncestorAttribute(input, "wg-notranslate") && input.type != "hidden") {
                    if (input.placeholder.trim() != "" && isNaN(input.placeholder.trim())) {
                        ...
                        b.push(input);
                        c.push(3);
                        d.push({t: 3, w: input.placeholder})
                    }
                    if (input.value.trim() != "" && isNaN(input.value.trim())) {
                        a.push(input.value);
                        b.push(input);
                        c.push(2);
                        d.push({t: 2, w: input.value})
                    }
                }
            }
            ...
            for (var i = 0; i < textareas.length; i++) {
                ...
                if (!hasAncestorAttribute(textarea, "wg-notranslate")) {
                    if (textarea.placeholder.trim() != "" && isNaN(textarea.placeholder.trim())) {
                        ...
                        b.push(textarea);
                        c.push(3);
                        d.push({t: 3, w: textarea.placeholder})
                    }
                    if (textarea.value.trim() != "" && isNaN(textarea.value.trim())) {
                        a.push(textarea.value);
                        b.push(textarea);
                        c.push(1);
                        d.push({t: 1, w: textarea.value})
                    }
                }
            }
        }
        return {values: a, nodes: b, types: c, payload: d}
    }
    ...
    function hasAncestorAttribute(el, attr) {
        ...
        if (el && el.nodeType === 1 && el.hasAttribute(attr)) return true; else return false
    }
    ...
    function translatePage(res, options, title_page) {
        ...
        if (options.transition == "wait") body.style.visibility = "hidden";
        var nodes = res.nodes;
        var types = res.types;
        ...
        if (options.originalLanguage == options.destinationLanguage) {
            for (var i = 0; i < nodes.length; i++) {
                try {
                    ...
                    if (types[i] == 1) nodes[i].textContent = translated_word; else if (types[i] == 3) nodes[i].placeholder = translated_word; else if (types[i] == 2) nodes[i].value = translated_word; else if (types[i] == 6) nodes[i].src = translated_word
                } catch (e) {
                    console.log(e)
                }
            }
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0)) {
                    ...
                    if (options.transition == "wait") body.style.visibility = "visible";
                    ...
                    for (var i = 0; i < jsoncount(nodes); i++) {
                        try {
                            ...
                            if (typeof translated_word === "undefined") {
                                if (types[i] == 1) translated_word = nodes[i].textContent; else if (types[i] == 3) translated_word = nodes[i].placeholder; else if (types[i] == 2) translated_word = nodes[i].value; else if (types[i] == 6) translated_word = nodes[i].src
                            }
                            if (types[i] == 1) nodes[i].textContent = translated_word; else if (types[i] == 3) nodes[i].placeholder = translated_word; else if (types[i] == 2) nodes[i].value = translated_word; else if (types[i] == 6) nodes[i].src = translated_word
                        } catch (e) {
                            console.log(e)
            ...
            var data = '{"l_from":"' + options.originalLanguage + '","l_to":"' + options.destinationLanguage + '","title":' + JSON.stringify(title_page) + ',"request_url":"' + window.location.href + '","words":' + JSON.stringify(payload) + "}";
            xhr.send(data)
        }
    }
    ...
    function translateElem(res, options, l_to, title_page) {
        ...
        xhr.open("POST", apiurl, true);
        xhr.setRequestHeader("Content-type", "application/json");
        var data = '{"l_from":"' + options.originalLanguage + '","l_to":"' + l_to + '","title":' + JSON.stringify(title_page) + ',"request_url":"' + window.location.href + '","words":' + JSON.stringify(payload) + "}";
        xhr.send(data)
    }
    ...
    function getElementByXpath(xPath, doc) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (i < arr.length - 1) prevElem = prevElem.getElementsByTagName(elem)[elemNum]; else return prevElem.getElementsByTagName(elem)[elemNum]
        }
    }
    ...
    function addLangSwitcher(res, options, title_page) {
        ...
        switcherWrap.innerHTML = '<aside style="display:none;" wg-notranslate="" id="weglot_switcher" class="country-selector closed" onclick="Weglot.openClose(this);"></aside>';
        ...
        for (index = 0; index < languages.length; index++) {
            ...
            if (l == options.originalLanguage) continue;
        ...
        openClose: function openClose(link) {
            ...
            document.addEventListener("DOMContentLoaded", function (event) {
                ...
                observeDOM(document.getElementsByTagName("body")[0], function (mutation) {
                    var all = {values: [], nodes: [], types: [], payload: []};
